...
if(isset($_POST['search']))
{
    ...
    ?>
         ...
         <table id="example1" class="table table-responsive ps ">
                  ...
                  while($row=$result->fetch_assoc())
                  {
                    ...
                    <td>".$row['reg_date']."</td>
                      <td>".$row['status']."</td>
                  <td> <a href='speci.php?asid=$row[id]' class='btn btn-info'>Approve<a>
                   <a href='speci.php?rjsid=$row[id]' class='btn btn-danger'>Reject<a></td>
                   </tbody>
                    "
                  ;}
  ...
  {
    $id=$_GET['asid'];
    $result=$mysqli->query("UPDATE request SET status='approved'  where id='$id'")or die($mysqli->error);
