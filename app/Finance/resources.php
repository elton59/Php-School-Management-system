...
if(isset($_POST['search']))
{
  ...
  <table id="example1" class="table table-responsive ps ">
          <h1> Resource Allocation</h1>
                      <thead>
                        <tr><th> ID</th><th> EXERCISE BOOKS</th><th>TEXT BOOKS  </th><th>PENS</th><th>PENCIL</th><th>CALCULATORS</th><th>LEVEL</th><th>DATE OF ISSUE</th><th>STATUS</th></tr>
                      </thead>
                  ...
                  while($row=$result->fetch_assoc())
                  {
                    ...
                    <td>".$row['level']."</td>
                     <td>".$row['reg_date']."</td>
                     <td>".$row['status']."</td>
                      <td> <a href='resources.php?asid=$row[id]' class='btn btn-info'>Approve<a>
                   <a href='resources.php?rjsid=$row[id]' class='btn btn-danger'>Reject<a></td>
  ...
  {
    $id=$_GET['asid'];
    $result=$mysqli->query("UPDATE resources SET STATUS='approved'  where id='$id'")or die($mysqli->error);
    
  }   
   if(isset($_GET['rjsid']))
  {
    $id=$_GET['rjsid'];
    $result=$mysqli->query("UPDATE resources SET STATUS='rejected'  where id='$id'")or die($mysqli->error);
