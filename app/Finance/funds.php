...
if(isset($_POST['search']))
{
    ...
    ?>
         ...
         <table id="example1" class="table table-responsive ps ">
          <h1> Fund Allocation</h1>
                      <thead>
                        <tr><th> ID</th><th> SCHOOL_NAME</th><th>SCHOOL_CODE</th><th>NUMBER OF STUDENTS</th><th>AMOUNT TO BE ALLOCATED</th><th>REGDATE</th><th>STATUS</th><th>ACTION</th></tr>
                      </thead>
                  ...
                  while($row=$result->fetch_assoc())
                  {
                    ...
                    <td>".$row['reg_date']."</td>
                    <td>".$row['status']."</td>
                    <td> <a href='funds.php?asid=$row[id]' class='btn btn-info'>Approve<a>
                   <a href='funds.php?rjsid=$row[id]' class='btn btn-danger'>Reject<a></td>
  ...
  {
    $id=$_GET['asid'];
    $result=$mysqli->query("UPDATE funds SET STATUS='approved'  where id='$id'")or die($mysqli->error);
    
  }   
   if(isset($_GET['rjsid']))
  {
    $id=$_GET['rjsid'];
    $result=$mysqli->query("UPDATE funds SET STATUS='rejected'  where id='$id'")or die($mysqli->error);
